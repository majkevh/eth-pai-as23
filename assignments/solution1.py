...
class Model(object):
    ...
    def __init__(self):
        ...
        """
        self.n_clusters = 7000 # to reduce the sample size
        self.model = GaussianProcessRegressor(kernel=Matern(nu=2.5) + WhiteKernel(),
    ...
    def make_predictions(self, test_x_2D: np.ndarray, test_x_AREA: np.ndarray) -> typing.Tuple[
        ...
        predictions = gp_mean + gp_std * test_x_AREA  # adding std to overestimate predictions on residential areas
    ...
    def fitting_model(self, train_y: np.ndarray, train_x_2D: np.ndarray):
        ...
        """
        data_sampled = KMeans(n_clusters=self.n_clusters, random_state=42).fit(train_x_2D)
        train_y_sampled = pd.DataFrame(train_y,data_sampled.labels_).groupby(data_sampled.labels_).mean().to_numpy()
        # mean pollution for each cluster
        print("Clustering completed, starting model training...")

        self.model.fit(data_sampled.cluster_centers_, train_y_sampled)
...
def main():
    ...
    print('Fitting model')
    model = Model()
    model.fitting_model(train_y, train_x_2D, train_x_AREA)

    # Predict on the test features
    print('Predicting on test features')
    predictions = model.make_predictions(test_x_2D, train_x_AREA)
    print(predictions)
