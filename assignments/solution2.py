...
class SWAGInference(object):
    ...
    def __init__(
        ...
        self.attributes = {}
        for name, param in self._create_weight_copy().items():
            self.attributes["{}s_mean".format(name)] = param.new(param.size()).zero_()
            self.attributes["{}s_sq_mean".format(name)] = param.new(param.size()).zero_()
        ...
        #  Hint: check collections.deque
            self.attributes["{}s_cov_mat_sqrt".format(name)] = param.new_empty((0, param.numel())).zero_()

        # Calibration, prediction, and other attributes
        # TODO(2): create additional attributes, e.g., for calibration
        self._prediction_threshold = None  # this is an example, feel free to be creative

    def update_swag(self, epoch) -> None:
        """
        ...
        for name, param in current_params.items():
            ...
            mean = self.attributes["{}s_mean".format(name)]
            sq_mean = self.attributes["{}s_sq_mean".format(name)]
            self.attributes["{}s_mean".format(name)] = (epoch*mean + param)/(epoch+1)
            self.attributes["{}s_sq_mean".format(name)] = (epoch*sq_mean + param**2)/(epoch+1)
        ...
        # Full SWAG
            if self.inference_mode == InferenceMode.SWAG_FULL:
                ...
                cov_mat_sqrt = self.attributes["{}s_cov_mat_sqrt".format(name)]
                dev = (param - mean).view(-1,1)
                cov_mat_sqrt = torch.cat((cov_mat_sqrt, dev.view(-1,1).t()), dim=0)
                self.attributes["{}s_cov_mat_sqrt".format(name)] = cov_mat_sqrt
    ...
    def fit_swag(self, loader: torch.utils.data.DataLoader) -> None:
        ...
        for name, param in self._create_weight_copy().items():
            self.attributes["{}s_mean".format(name)] = self.network.state_dict()[name].data
            self.attributes["{}s_sq_mean".format(name)] = self.network.state_dict()[name].data**2
    ...
    def predict_probabilities_swag(self, loader: torch.utils.data.DataLoader) -> torch.Tensor:
        ...
        sum = 0
        for tens in per_model_sample_predictions:
            sum = sum + tens
        bma_probabilities = (1/self.bma_samples)*sum
    ...
    def sample_parameters(self) -> None:
        ...
        for name, param in self.network.named_parameters():
            ...
            current_mean = self.attributes["{}s_mean".format(name)]
            current_sq_mean = self.attributes["{}s_sq_mean".format(name)]
            current_std = 1/math.sqrt(2)*torch.sqrt(torch.clamp(current_sq_mean-current_mean**2,
                                                           1e-30))
            ...
            if self.inference_mode == InferenceMode.SWAG_FULL:
                ...
                current_cov_mat_sqrt = self.attributes["{}s_cov_mat_sqrt".format(name)]
                z_2 = current_cov_mat_sqrt.new_empty((current_cov_mat_sqrt.size(0), 1)).normal_()
                sampled_param += (1/np.sqrt(2*(self.swag_epochs - 1)))* \
                                 current_cov_mat_sqrt.t().matmul(z_2).view_as(current_mean)
